...
function translateToSql(adhocMetric, { useSimple } = {}) {
  if (adhocMetric.expressionType === EXPRESSION_TYPES.SIMPLE || useSimple) {
    ...
    return `${adhocMetric.subject} ${adhocMetric.operator === '==' ? '=' : adhocMetric.operator} ${isMulti ? '(\'' : ''}${isMulti ? adhocMetric.comparator.join("','") : adhocMetric.comparator}${isMulti ? '\')' : ''}`;
...
export default class AdhocFilter {
  constructor(adhocFilter) {
    ...
    if (this.expressionType === EXPRESSION_TYPES.SIMPLE) {
      ...
      this.sqlExpression = adhocFilter.sqlExpression || translateToSql(adhocFilter, { useSimple: true });
      this.clause = adhocFilter.clause;
      this.subject = null;
      this.operator = null;
  ...
  isValid() {
    if (this.expressionType === EXPRESSION_TYPES.SIMPLE) {
      return !!(this.operator && this.subject && this.comparator);
    } else if (this.expressionType === EXPRESSION_TYPES.SQL) {
